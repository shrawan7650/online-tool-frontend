...
export function HashPage() {
    ...
    async (e?: React.FormEvent) => {
      ...
      try {
        ...
        if (selectedAlgorithm === "md5") {
          ...
          newResults[selectedAlgorithm] = await generateHash(
            input,
            selectedAlgorithm,
          );
        }
        ...
        toast.success(
          `${selectedAlgorithm.toUpperCase()} hash generated successfully`,
        );
      } catch (error) {
  ...
  const algorithms: {
  ...
  }[] = [
    {
      value: "md5",
      label: "MD5",
      description: "128-bit hash (legacy, not cryptographically secure)",
    },
    {
      value: "sha1",
      label: "SHA-1",
      description: "160-bit hash (legacy, use SHA-256+ for security)",
    },
    {
      value: "sha256",
      label: "SHA-256",
  ...
  return (
    <div className="max-w-4xl px-4 py-8 mx-auto sm:px-6 lg:px-8 animate-fade-in">
        ...
        description="Generate cryptographic hashes online. Support for MD5, SHA-1, SHA-256, and SHA-512 algorithms. Fast, secure, and client-side processing."
        keywords="hash generator, MD5, SHA1, SHA256, SHA512, cryptographic hash, checksum, file integrity"
        canonicalUrl="/hash"
      />
      <div className="mb-8 text-center">
        ...
        <p className="text-lg text-slate-400">
          Generate secure cryptographic hashes using MD5, SHA-1, SHA-256, and
          SHA-512 algorithms
        </p>
      </div>
              ...
              {results[algo.value] ? (
                <div className="flex items-center space-x-3">
                  ...
                  <button
                    ...
                    disabled={
                      !results[algo.value] ||
                      results[algo.value].includes("Use backend")
                    }
                  >
      ...
      <div className="mt-8 tool-card">
        ...
        <div className="prose prose-invert max-w-none">
          <p className="mb-4 text-slate-400">
            Cryptographic hash functions are mathematical algorithms that map
            data of arbitrary size to a fixed-size string. They are designed to
            be one-way functions, making it computationally infeasible to
            reverse the process.
          </p>
          ...
          <div className="p-4 mt-6 border rounded-lg bg-red-900/20 border-red-700/30">
            ...
            <ul className="space-y-1 text-sm text-red-200">
              <li>
                MD5 and SHA-1 are cryptographically broken - use SHA-256 or
                better
              </li>
              <li>
                Never use plain hashes for passwords - always use salt + slow
                hash functions
              </li>
              <li>
                Hashing alone doesn't provide authentication or encryption
              </li>
            </ul>
          </div>
        </div>
